d3 = function() {
  ...
  var iStar = Math.SQRT2, iStar2 = 2, iStar4 = 4;
  d3.interpolateZoom = function(p0, p1) {
    var ux0 = p0[0], uy0 = p0[1], w0 = p0[2], ux1 = p1[0], uy1 = p1[1], w1 = p1[2];
    var dx = ux1 - ux0, dy = uy1 - uy0, d2 = dx * dx + dy * dy, d1 = Math.sqrt(d2), b0 = (w1 * w1 - w0 * w0 + iStar4 * d2) / (2 * w0 * iStar2 * d1), b1 = (w1 * w1 - w0 * w0 - iStar4 * d2) / (2 * w1 * iStar2 * d1), r0 = Math.log(Math.sqrt(b0 * b0 + 1) - b0), r1 = Math.log(Math.sqrt(b1 * b1 + 1) - b1), dr = r1 - r0, S = (dr || Math.log(w1 / w0)) / iStar;
    function interpolate(t) {
      var s = t * S;
      if (dr) {
        var coshr0 = d3_cosh(r0), u = w0 / (iStar2 * d1) * (coshr0 * d3_tanh(iStar * s + r0) - d3_sinh(r0));
        return [ ux0 + u * dx, uy0 + u * dy, w0 * coshr0 / d3_cosh(iStar * s + r0) ];
      }
      return [ ux0 + t * dx, uy0 + t * dy, w0 * Math.exp(iStar * s) ];
    }
  ...
  function d3_geo_conicEqualArea(φ0, φ1) {
    var sinφ0 = Math.sin(φ0), n = (sinφ0 + Math.sin(φ1)) / 2, C = 1 + sinφ0 * (2 * n - sinφ0), iStar0 = Math.sqrt(C) / n;
    function forward(λ, φ) {
      var iStar = Math.sqrt(C - 2 * n * Math.sin(φ)) / n;
      return [ iStar * Math.sin(λ *= n), iStar0 - iStar * Math.cos(λ) ];
    }
    forward.invert = function(x, y) {
      var iStar0_y = iStar0 - y;
      return [ Math.atan2(x, iStar0_y) / n, d3_asin((C - (x * x + iStar0_y * iStar0_y) * n * n) / (2 * n)) ];
    };
    return forward;
  }
  ...
  function d3_geo_azimuthal(scale, angle) {
    ...
    azimuthal.invert = function(x, y) {
      var iStar = Math.sqrt(x * x + y * y), c = angle(iStar), sinc = Math.sin(c), cosc = Math.cos(c);
      return [ Math.atan2(x * sinc, iStar * cosc), Math.asin(iStar && y * sinc / iStar) ];
    };
    return azimuthal;
  }
  ...
  function d3_geo_conicConformal(φ0, φ1) {
    ...
    function forward(λ, φ) {
      var iStar = abs(abs(φ) - halfπ) < ε ? 0 : F / Math.pow(t(φ), n);
      return [ iStar * Math.sin(n * λ), F - iStar * Math.cos(n * λ) ];
    }
    forward.invert = function(x, y) {
      var iStar0_y = F - y, iStar = d3_sgn(n) * Math.sqrt(x * x + iStar0_y * iStar0_y);
      return [ Math.atan2(x, iStar0_y) / n, 2 * Math.atan(Math.pow(F / iStar, 1 / n)) - halfπ ];
    };
    return forward;
  }
  ...
  function d3_geo_conicEquidistant(φ0, φ1) {
    ...
    function forward(λ, φ) {
      var iStar = G - φ;
      return [ iStar * Math.sin(n * λ), G - iStar * Math.cos(n * λ) ];
    }
    forward.invert = function(x, y) {
      var iStar0_y = G - y;
      return [ Math.atan2(x, iStar0_y) / n, G - d3_sgn(n) * Math.sqrt(x * x + iStar0_y * iStar0_y) ];
    };
    return forward;
  }
